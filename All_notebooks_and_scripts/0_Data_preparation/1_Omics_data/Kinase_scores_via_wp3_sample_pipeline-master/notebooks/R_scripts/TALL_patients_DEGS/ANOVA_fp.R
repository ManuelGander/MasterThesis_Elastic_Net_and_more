# Findind differentially expressed genes in the TALL patients with respect to other batches
library(ggvenn)
library(magrittr)
filePath = '/media/kusterlab/internal_projects/active/TOPAS/WP31/Playground/Retrospective_study/2022.11.28_AhS_all_mixed_cohort/full_proteome_measures_z.tsv'
...
rownames(TALL_z_scores) = z_scors$Gene.names
...
x <- list(
  BCR_ABL1_proteins = BCR_ABL1_proteins, 
  IgH_CRLF2_proteins = IgH_CRLF2_proteins, 
  ETV_RUNX1_proteins = ETV_RUNX1_proteins
...
int = p_values[p_values$p_values < .01 & p_values$p.adjusted_fdr <0.2 ,]
...
write.csv(p_values,'/media/kusterlab/internal_projects/active/TOPAS/WP31/Playground/Retrospective_study/2022.11.28_AhS_all_mixed_cohort/ANOVA_DEGs.csv')
