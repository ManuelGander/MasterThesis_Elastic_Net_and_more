...
def calculate_psite_weights(df: pd.DataFrame):
    ...
    patient_weight_columns = [col.replace('pat_', 'weight_') for col in patient_zscore_columns]
    ...
    weights_df[patient_weight_columns] = weights_df[patient_weight_columns].notna().mul(df['Peptide count'], axis=0)
...
def calculate_modified_sequence_weights(patient_dataframe: pd.DataFrame, summing_column: str):
    #patient_dataframe.to_csv('/home/mgander/Atlantic/test.csv')
...
def cap_zscores_and_weights(patient_dataframe):
    ...
    capped_dataframe[weightcols] = capped_dataframe[weightcols].astype(float).clip(upper=1)
    # capped_dataframe[weightcols] = capped_dataframe[weightcols].astype(float)
    # TODO: Test if this capping step is required too
    capped_dataframe[patcols] = capped_dataframe[patcols].astype(float).clip(upper=4, lower=-4)
...
def calculate_weighted_z_scores(df: pd.DataFrame):
    ...
    weighted_zscores_df = df[patient_zscore_columns].mul(df[patient_weight_columns].rename(columns=lambda x: x.replace('weight_', 'pat_')))
    weighted_zscores_df = weighted_zscores_df.rename(columns=lambda x: x.replace('pat_', 'weighted_'))

    return pd.concat([df, weighted_zscores_df], axis=1)


def sum_weighted_z_scores(patient_dataframe, by):
    relevant_columns = [col for col in patient_dataframe.columns if ('weight_' not in col) and ('pat_' not in col)]
    ...
    score_dataframe = score_dataframe.rename(
        columns={col: col.replace('weighted_', '') for col in score_dataframe.columns})
...
def plot_histograms_to_check_normality(score_dataframe, number_of_plots=20):
    ...
    for row in random.sample(range(len(score_dataframe)), number_of_plots):
        ...
        plt.hist(zscores, weights=np.zeros_like(zscores) + 1. / zscores.size)
        xmin, xmax = plt.xlim()
...
def second_level_z_scoring(patient_dataframe, by_column, plot_histograms=False):
    patient_dataframe['mean'] = patient_dataframe[
        [col for col in patient_dataframe.columns if col not in [by_column, 'Modified sequence']]].mean(axis=1)
    patient_dataframe['stdev'] = patient_dataframe[
        [col for col in patient_dataframe.columns if col not in [by_column, 'Modified sequence', 'mean']]].std(axis=1)
    for patient in patient_dataframe.columns:
        if patient in [by_column, 'Modified sequence', 'mean', 'stdev']:
            continue
        patient_dataframe[patient] = (patient_dataframe[patient] - patient_dataframe['mean']) / patient_dataframe[
            'stdev']
    if plot_histograms:
...
def calculate_peptide_occurrence(pp_df: pd.DataFrame, patient_regex):
    pp_df = pp_df.replace('nan', np.nan)
    pp_df = pp_df.replace('', np.nan)
    patient_list = pp_df.filter(regex=patient_regex).columns.tolist()
...
def topas_score_preprocess(results_folder, patient_regex, discard_isoforms=True):
    ...
    patients_proteins = pd.read_csv(os.path.join(results_folder, 'annot_pp.csv'), index_col=0)
    ...
    patients_zscores = pd.read_csv(os.path.join(results_folder, 'phospho_measures_z.tsv'), keep_default_na=False, sep='\t')
    ...
    patients = calculate_peptide_occurrence(patients, patient_regex=patient_regex)  # better name?
...
def calculate_per_patient_targets(scored_peptide_df: pd.DataFrame, grouping_by: str):
    ...
    per_patient_targets = scored_peptide_df.groupby(grouping_by)[patients].count().reset_index()
    per_patient_targets.rename(columns={col: col.replace('pat_', 'targets_') for col in per_patient_targets.columns if 'pat_' in col}, inplace=True)
    return per_patient_targets


def get_target_space(annotated_peptides_df, grouping_by, scored_peptides_df):
    total_targets = annotated_peptides_df.groupby(grouping_by)['Modified sequence'].nunique().reset_index(
        name='No. of total targets')
    per_patient_targets = calculate_per_patient_targets(scored_peptides_df, grouping_by)
    target_space = pd.merge(left=total_targets, right=per_patient_targets, on=grouping_by, how='inner', validate='1:1')
    return target_space
