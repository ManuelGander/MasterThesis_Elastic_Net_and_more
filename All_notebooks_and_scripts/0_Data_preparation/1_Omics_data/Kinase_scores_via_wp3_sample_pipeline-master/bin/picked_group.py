...
from picked_group_fdr.quant.peptide_count import UniquePeptideCountColumns
...
def prepare_evidence_for_picked(df, evidence_file):
    ...
    info_cols = ['Modified sequence', 'Proteins', 'Leading proteins', 'Gene names', 'Raw file', 'Fraction', 'Batch', 'Charge',
                 'PEP', 'id']
    if 'Gene names' not in df.columns:
        info_cols.remove('Gene names')
    intensity_cols = ['Intensity'] + df.columns[df.columns.str.startswith('Reporter intensity')].tolist()
...
def run_picked_group_fdr(evidence_file, picked_fdr_file, fasta_file, gene_level: bool = True):
    if os.path.exists(picked_fdr_file):
        logger.info(f"Reusing previously generated results from picked protein group FDR")
        return

    logger.info("Running picked group FDR")

    picked.main(['--mq_evidence', evidence_file,
                 '--protein_groups_out', picked_fdr_file,
                 '--fasta', fasta_file,
                 '--enzyme', 'trypsinp',
                 '--min-length', '6',
                 '--cleavages', '3',
...
def picked_protein_grouping(
    ...
    logging.getLogger('picked_group_fdr').handlers = logging.getLogger(__package__).handlers

    # Picked protein grouping
    evidence_file = f'{results_folder}/{evidence_file_base}.txt'

    if os.path.exists(evidence_file):
        logger.info(f"Reusing previously generated evidence file for picked protein group FDR")
    else:
    ...
    do_quant(df, picked_fdr_file, fasta_file, picked_fdr_file_with_quant, fdr_num_threads)
    ...
    df = df.rename(columns=lambda x: x.replace('LFQ Intensity Reporter intensity', 'Reporter intensity'))

    return df


def do_quant(df, picked_fdr_file, fasta_file, picked_fdr_file_with_quant, fdr_num_threads: str):
    if os.path.exists(picked_fdr_file_with_quant):
        logger.info(f"Reusing previously generated results from picked protein group FDR with quantification")
        return

    args = quant.parseArgs(['--mq_evidence', 'dummy',
                            '--mq_protein_groups', 'dummy',
                            '--protein_groups_out', 'dummy',
                            '--fasta', fasta_file,
                            '--enzyme', 'trypsinp',
                            '--min-length', '6',
                            '--cleavages', '3',
    ...
    peptideToProteinMap, _ = quant.getPeptideToProteinMaps(args, parseId)

    # read in the protein groups from the protein grouping step
    proteinGroupResults = ProteinGroupResults.from_mq_protein_groups_file(picked_fdr_file)
    ...
    proteinGroupResults, parsedExperiments = add_peptides_to_protein_groups(df, peptideToProteinMap, proteinGroupResults,
                                                                            proteinGroups)
    ...
    columns = [UniquePeptideCountColumns(),
               LFQIntensityColumns(silacChannels=[], minPeptideRatiosLFQ=args.lfq_min_peptide_ratios,
                                   stabilizeLargeRatiosLFQ=args.lfq_stabilize_large_ratios, numThreads=int(fdr_num_threads))]
    ...
    logger.info(f"Protein group results have been written to: {picked_fdr_file_with_quant}")


def add_peptides_to_protein_groups(df, peptideToProteinMap, proteinGroupResults, proteinGroups):
    ...
    num_tmt_channels = len(df.filter(regex='Reporter_intensity_corrected_', axis='columns').columns)
    for row in df.itertuples():  # itertuples is ~5x faster than iterrows
        proteins = digest.getProteins(peptideToProteinMap, helpers.cleanPeptide(row.Modified_sequence))
        ...
        for tmt_channel in range(1, num_tmt_channels + 1):
            ...
            precursorQuant = PrecursorQuant(row.Modified_sequence,
                                            row.Charge,
                                            experiment,
                                            row.Fraction,
                                            intensity,
    ...
    if missingPeptidesInFasta > 0:
        logger.info(f"Skipped {missingPeptidesInFasta} precursors not present in the fasta file")
    return proteinGroupResults, parsedExperiments
...
def remap_gene_names(df: pd.DataFrame, fasta_file: str) -> pd.DataFrame:
    ...
    proteinAnnotations = digest.getProteinAnnotations(fasta_file, parseId)
    ...
    def get_gene_names(protein_ids: str) -> str:
        ...
        """
        gene_names = [proteinAnnotations.get(protein_id, ("", "", ""))[1] for protein_id in protein_ids.split(';')]
        gene_names = {g for g in gene_names if len(g) > 0}
