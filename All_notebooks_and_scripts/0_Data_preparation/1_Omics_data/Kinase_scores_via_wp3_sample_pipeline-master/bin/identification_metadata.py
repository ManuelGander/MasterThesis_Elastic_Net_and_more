...
def get_metadata_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(metadata_df.columns) == 0:
        raise ValueError(f"Could not find '{METADATA_COLUMN_PREFIX}' columns in the dataframe")
    return metadata_df
...
def mark_detected_in_batch(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for batch in batches:
        tmt_cols_df = df.filter(regex=fr'^Reporter intensity corrected \d{{1,2}} {batch}$', axis='columns')
        ...
        df[imputation_cols] += np.where(tmt_cols_df.isna().multiply(num_detected_in_batch > 0, axis='index'), 'detected in batch;', '')
...
def replace_detected_in_batch(df: pd.DataFrame, annot_df: pd.DataFrame, value: float) -> pd.DataFrame:
    ...
    detected_in_batch_df = detected_in_batch_df.replace(False, np.nan).replace(True, value)
    df.update(detected_in_batch_df, overwrite=False)  # for some reason update can only be done in place
    return df


def mark_num_peptides(df: pd.DataFrame) -> pd.DataFrame:
    logger.info("Mark number of peptide identifications per sample")
    peptide_counts_df = df.filter(regex=fr'^Unique peptides Reporter intensity corrected \d{{1,2}}', axis='columns')
    imputation_cols = as_metadata_columns(peptide_counts_df.columns.str.replace('Unique peptides ', '').str)
    
    def num_peptide_string(peptide_count_cols):
        return [f"num_peptides={int(c)};" if c > 0 else "" for c in peptide_count_cols]

    df[imputation_cols] += peptide_counts_df.apply(num_peptide_string, axis=1, result_type="expand").values
    return df


def get_num_peptides(df: pd.DataFrame) -> pd.DataFrame:
    metadata_df = get_metadata_columns(df)
    return metadata_df.apply(lambda x: x.str.extract('num_peptides=(\d+);')[0], axis=1).astype(float)


def filter_by_min_peptides(df: pd.DataFrame, annot_df: pd.DataFrame, min_peptides: int) -> pd.DataFrame:
    """Replace nan values for peptides/proteins detected in a batch by a value"""
    detected_in_batch_df = get_num_peptides(annot_df)
    detected_in_batch_df = remove_metadata_column_prefix(detected_in_batch_df)
    detected_in_batch_df = detected_in_batch_df[df.columns]
    detected_in_batch_df = detected_in_batch_df.applymap(lambda x: -1 if x < min_peptides else np.nan)
    df.update(detected_in_batch_df, overwrite=True)  # for some reason update can only be done in place
