...
def read_basket_scores(results_folder: Union[str, Path],
                       data_type: str = 'None', main_basket_only: bool = True,
                       z_scored: bool = False) -> pd.DataFrame:
    """
    ...
    for basket_score_file_name in [f'basket_scores_4th_gen{z_scored_suffix}.tsv', 'basket_scores{z_scored_suffix}.tsv']:
        basket_scores_file_path = os.path.join(results_folder, basket_score_file_name)
        if not os.path.exists(basket_scores_file_path):
            continue
        
        try:
            basket_scores_df = pd.read_csv(basket_scores_file_path, sep='\t', index_col='Sample')
        except PermissionError:
            raise PermissionError(f'Cannot open basket scores file, check if you have it open in Excel. {basket_scores_file_path}')
        ...
        if basket_score_file_name == "basket_scores.tsv":
            if data_type != 'None':
                try:
                    ...
                    raise ValueError(f'Data type given for subset is not found in basket score file: {data_type}')
            # remove drug scores
    ...
    for file in sub_basket_files:
        ...
        if os.path.exists(sub_basket_file):
            try:
                ...
                raise PermissionError(f'Cannot open subbasket scores file, check if you have it open in Excel. {sub_basket_file}')
        else:
            raise FileNotFoundError(f'Subbaskets score file not found. {sub_basket_file}')
        ...
        df = df.drop(df.loc[:, df.columns.str.contains('total_basket_score|Sample|subtype|Subtype')], axis=1)
...
def get_paths_to_sub_basket_files(results_folder):
    files = [filename for filename in os.listdir(results_folder) if filename.startswith('subbasket_scores')]
    return files


def compute_basket_scores(results_folder: Union[str, Path],
                          patient_regex: str,
                          debug: bool,
                          data_types: List[str],
    ...
    """
    Computes basket scores
    Requires that [phospho|full_proteome]_[z|rank|p|fc].csv files are generated by metrics.py
    """
    logger.info('Running basket scoring module')

    if "3rd gen" in baskets_file:
        if os.path.exists(os.path.join(results_folder, 'basket_scores.tsv')):
            logger.info(f'Basket scoring skipped - found files already preprocessed')
            return
        compute_basket_scores_3rd_gen(results_folder, debug, data_types)
    elif "4th gen" in baskets_file:
        if os.path.exists(os.path.join(results_folder, 'basket_scores_4th_gen.tsv')):
            logger.info(f'Basket scoring skipped - found files already preprocessed')
            return
        # TODO: implement that debug is used and files with ref can be saved
        compute_basket_scores_4th_gen(results_folder, patient_regex, debug, metadata_file, baskets_file, data_types)
    else:
        raise ValueError("Unknown baskets generation")


def load_z_scores_fp(results_folder, patient_regex):
    z_scores_fp_df = metrics.read_measures(results_folder, 'fp', [('z', 'z-score')])['z-score']
    z_scores_fp_df = z_scores_fp_df.rename(columns=lambda x: x.replace("zscore_", ""))
    z_scores_fp_df = utils.keep_only_sample_columns(z_scores_fp_df, patient_regex)
...
def load_z_scores_pp(results_folder, patient_regex):
    z_scores_pp_df = metrics.read_measures(results_folder, 'pp', [('z', 'z-score')])['z-score']
    z_scores_pp_df = z_scores_pp_df.rename(columns=lambda x: x.replace("zscore_", ""))

    z_scores_pp_df = z_scores_pp_df.reset_index()
    z_scores_pp_df['Modified sequence'] = utils.short_phospho_notation(z_scores_pp_df['Modified sequence'])
    z_scores_pp_df = z_scores_pp_df.set_index('Modified sequence')

    z_scores_pp_df = utils.keep_only_sample_columns(z_scores_pp_df, patient_regex)

    return z_scores_pp_df


def load_protein_phosphorylation(results_folder, patient_regex):
    ...
    protein_phosphorylation_df = utils.keep_only_sample_columns(protein_phosphorylation_df, patient_regex)
    return protein_phosphorylation_df


def load_kinase_scores(results_folder, patient_regex: str):
    kinase_scores_df = pd.read_csv(os.path.join(results_folder, 'kinase_results', 'kinase_scores.tsv'), sep='\t', index_col='PSP Kinases')
    kinase_scores_df = utils.keep_only_sample_columns(kinase_scores_df, patient_regex)
    return kinase_scores_df


def compute_basket_scores_4th_gen(results_folder: Union[str, Path],
                                  patient_regex: str,
                                  debug: bool,
                                  metadata_file: Union[str, Path],
                                  baskets_file: Union[str, Path],
    ...
    """
    Computes subbasket and basket scores
    Requires that
    ...
    """
    # TODO: find a way to read in only necessary columns - patients, basket+subbasket --> not metadata
    metadata_df = sample_metadata.load(metadata_file)
    all_baskets_df = read_baskets_file_4th_gen(baskets_file)
    biomarkers = all_baskets_df[all_baskets_df['GROUP'] == 'OTHER']
    all_baskets_df = all_baskets_df[all_baskets_df['GROUP'] != 'OTHER']

    z_scores_fp_df = load_z_scores_fp(results_folder, patient_regex)
    z_scores_pp_df = load_z_scores_pp(results_folder, patient_regex)
    protein_phosphorylation_df = load_protein_phosphorylation(results_folder, patient_regex)
    kinase_scores_df = load_kinase_scores(results_folder, patient_regex)

    total_basket_scores = {}
    for basket_name, basket_df in all_baskets_df.groupby('BASKET'):
        logger.info(f"Calculating basket scores for {basket_name}")
        subbasket_scores = {}
        for subbasket_name, subbasket_df in basket_df.groupby('subbasket_level'):
            ...
            if 'Ligand' in subbasket_name:
                ligand = True
            logger.info(f"- Calculating subbasket scores for {basket_name} - {subbasket_name} (scoring rule: {scoring_rule})")
            if scoring_rule == 'highest z-score':
                subbasket_score = get_weighted_max(z_scores_fp_df, subbasket_df, 'Gene names', 'GENE NAME', ligand)
            elif scoring_rule == 'highest z-score (p-site)':
                subbasket_score = get_weighted_max(z_scores_pp_df, subbasket_df, 'Modified sequence', 'MODIFIED SEQUENCE', ligand)
            elif scoring_rule == 'highest protein phosphorylation score (2nd level z-score, fh)':
                subbasket_score = get_weighted_max(protein_phosphorylation_df, subbasket_df, 'Gene names', 'GENE NAME', ligand)
            elif scoring_rule == 'highest kinase score (2nd level z-score, fh)':
                subbasket_score = get_weighted_max(kinase_scores_df, subbasket_df, 'PSP Kinases', 'GENE NAME', ligand)
            else:
        ...
        subbasket_scores_df[f'{basket_name}_total_basket_score'] = subbasket_scores_df.sum(axis=1)
        total_basket_scores[basket_name] = subbasket_scores_df[f'{basket_name}_total_basket_score']
        ...
        subbasket_output_file = os.path.join(results_folder,
        ...
        # subbasket_output_file_zscored = os.path.join(results_folder, f'subbasket_scores_{basket_name.replace("/", "_").replace(" ", "_")}_zscored.tsv')
        # subbasket_scores_df.to_csv(subbasket_output_file_zscored, sep='\t', index=False)

        logger.info(f"Written basket results for {basket_name} to: {subbasket_output_file}")

    # Biomarkers
    all_biomarkers = pd.DataFrame()
    for subbasket_name, subbasket_df in biomarkers.groupby('SUBBASKET'):
        logger.info(f"- Retrieving subbasket z-scores for Biomarker subbasket {subbasket_name}")
        subbasket_score = get_zscores(z_scores_fp_df, subbasket_df, 'Gene names', 'GENE NAME')
        all_biomarkers = pd.concat([all_biomarkers, subbasket_score], axis=0)
    all_biomarkers.to_csv(os.path.join(results_folder, 'biomarker_scores.tsv'), sep='\t')

    # add annotation per patient for # identifications and # annotations
    ident_annot_df = get_number_ident_annot_per_sample(results_folder, patient_regex, data_types)
    ident_annot_df.to_csv(os.path.join(results_folder, 'ident_annot.csv'))

    basket_scores_df = pd.DataFrame.from_dict(total_basket_scores)
    save_basket_scores(basket_scores_df, os.path.join(results_folder, 'basket_scores_4th_gen.tsv'))

    # apply second-level z-scoring per basket (i.e. per column)
    # basket_scores_df = basket_scores_df.apply(zscore)

    basket_scores_df = basket_scores_df.drop(basket_scores_df[basket_scores_df.index.str.startswith('targets')].index)
    measures = metrics.compute_measures(basket_scores_df.T, patient_regex)
    measures['z-score'].columns = measures['z-score'].columns.str.strip('zscore_')
    zscores = measures['z-score'].T

    save_basket_scores(zscores, os.path.join(results_folder, 'basket_scores_4th_gen_zscored.tsv'))


def get_number_ident_annot_per_sample(results_folder: Union[str, Path],
                                      patient_regex: str,
    ...
    for data_type in data_types:
        ...
        dfs[data_type] = pd.read_csv(os.path.join(results_folder, f'annot_{data_type}.csv'), index_col=index_col,
                                     keep_default_na=keep_default_na)
        # atm PP is read with missing values instead of nan which is why masking is necessary. TODO: just fix this
        dfs[data_type + '.num_identified'] = count_sample_identifications(dfs[data_type], patient_regex)
        tmt_channels = dfs[data_type].filter(regex=patient_regex).columns.tolist()
        dfs[data_type + '.num_annotated'] = dfs[data_type][tmt_channels + ['basket']].set_index('basket').apply(
            count_basket_annotations)
...
def count_sample_identifications(df, patient_regex: str):
    return df.filter(regex=patient_regex, axis=1).mask(df == '').count()
...
def count_significant_baskets(df_zscored: pd.DataFrame, thresholds: List[float] = [2.0]):
    """Computes number of significant basket scores per sample.

    Args:
        df_zscored: DataFrame with z-scores basket scores
    ...
    """    
    significant_baskets_per_patient = (df_zscored > thresholds[0]).sum(axis=1).to_frame('num_significant_baskets')
    if len(thresholds) > 1:
        for threshold in thresholds[1:]:
            significant_baskets_per_patient[f'num_significant_baskets_z>{threshold}'] = (df_zscored > threshold).sum(axis=1)
    # significant_baskets_per_patient['num_significant_baskets_z>1.2'] = (df_zscored > 1.2).sum(axis=1)
...
def basket_sheet_sanity_check(all_baskets_df: pd.DataFrame) -> None:
    ...
    valid_scoring_rules = {'highest z-score',
    ...
    scoring_rules_found = set(all_baskets_df[~all_baskets_df['MODIFIED SEQUENCE'].isnull()]['SCORING RULE'].str.lower().unique())
    valid_scoring_rules = {'highest z-score (p-site)'}
    unknown_scoring_rules = list(scoring_rules_found - valid_scoring_rules)
    if len(unknown_scoring_rules) > 0:
        raise ValueError(f"Invalid scoring rule for entry with modified sequence: {unknown_scoring_rules}")
...
def get_weighted_max(z_score_df: pd.DataFrame,
                     subbasket_df: pd.DataFrame,
                     z_score_index_col: str,
                     subbasket_index_col: str, ligand: bool) -> pd.Series:
...
def get_zscores(z_score_df: pd.DataFrame,
                subbasket_df: pd.DataFrame,
                z_score_index_col: str,
                subbasket_index_col: str):
    # only keep samples and z_score_index_col columns
    z_scores = z_score_df.astype(float)  # make sure all z-scores are floats and not objects
    ...
    z_scores, z_score_index_col_exploded = utils.explode_on_separated_string(z_scores, z_score_index_col)
    # merge z-score dataframe with basket genes
    z_scores = z_scores.merge(subbasket_df[[subbasket_index_col]], left_on=z_score_index_col_exploded,
...
def get_weighted_z_scores(z_score_df: pd.DataFrame,
                          subbasket_df: pd.DataFrame,
                          z_score_index_col: str,
                          subbasket_index_col: str) -> pd.DataFrame:
    """
    ...
    # merge z-score dataframe with basket genes
    z_scores = z_scores.merge(subbasket_df[[subbasket_index_col, 'WEIGHT']], left_on=z_score_index_col_exploded,
                              right_on=subbasket_index_col)
    ...
    z_scores = z_scores.drop(columns=[z_score_index_col_exploded, subbasket_index_col, 'WEIGHT'])
    z_scores = z_scores.multiply(weights, axis=0)
    return z_scores


def compute_basket_scores_3rd_gen(results_folder: Union[str, Path],
                                  debug: bool,
                                  data_types: List[str]) -> None:
    # ~12 minutes (~2 minutes for full proteome, ~6 minutes for phospho fold changes, ~4 minutes for phospho z-scores)
    basket_scores = pd.DataFrame()
    basket_scores_ref = pd.DataFrame()

    for data_type in data_types:
        logger.info(f'Reading in metrics for {data_type} data')
        measures = metrics.read_measures(results_folder, data_type)
        basket_scores = calculate_basket_scores(measures, basket_scores, data_type)
        if debug:
            measures_ref = metrics.read_measures(results_folder, data_type, with_reference_channels=True)
            basket_scores_ref = calculate_basket_scores(measures_ref, basket_scores_ref, data_type)

    basket_scores = save_basket_scores(basket_scores, os.path.join(results_folder, 'basket_scores.tsv'))
    if debug:
        save_basket_scores(basket_scores_ref, os.path.join(results_folder, 'basket_scores_ref.tsv'))

    save_basket_scores_long_format(basket_scores, os.path.join(results_folder, 'basket_scores_long.tsv'))


def calculate_basket_scores(measures: Dict[str, pd.DataFrame],
                            basket_scores: pd.DataFrame,
                            data_type: str,
                            score_method: str = 'sum_weighted_zscore') -> pd.DataFrame:
    ...
    with warnings.catch_warnings():
        ...
        basket_scores = calculate_basket_scores_single(measures['score'], f'{data_type.upper()} - Scores', 'basket',
                                                       basket_scores)
        basket_scores = calculate_basket_scores_single(measures['score'], f'{data_type.upper()} - RTK Scores', 'rtk',
                                                       basket_scores)

    return basket_scores


def calculate_basket_scores_single(score_df: pd.DataFrame,
                                   sheet_name: str,
                                   basket_column: str,
                                   scores: pd.DataFrame,
                                   apply_z_score=False):
    ...
    score_df[f'{basket_column}_weights'] = score_df[f'{basket_column}_weights'].apply(lambda x: x.split(";"))
    ...
    for basket, df_basket in score_df.groupby(basket_column):
        if 'FP' in sheet_name:
            df_basket[patient_columns] = df_basket[patient_columns].multiply(df_basket[f'{basket_column}_weights'].astype(float),
                                                                             axis="index")
        if sheet_name == 'FP - Scores' and basket_column == 'basket' and basket in ['TK', 'Immunotherapy']:
        ...
        if apply_z_score:
            scores[f"{sheet_name}.{basket}"] = (scores[f"{sheet_name}.{basket}"] - scores[f"{sheet_name}.{basket}"].median()) / \
                                               scores[
                                                   f"{sheet_name}.{basket}"].std()
    return scores
...
def save_basket_scores_long_format(basket_scores, out_file):
    basket_names = basket_scores.columns[basket_scores.columns != 'Sample'].values.tolist()
    basket_scores_long = pd.melt(basket_scores.reset_index(), id_vars='Sample', value_vars=basket_names)
    basket_scores_long['Data type'] = ['FP' if 'FP' in variable else 'PP' for variable in basket_scores_long['variable']]
    basket_scores_long['Basket type'] = ['RTK' if 'RTK Scores' in variable else 'Main' for variable in
                                         basket_scores_long['variable']]
    basket_scores_long = basket_scores_long.loc[~basket_scores_long['variable'].str.contains('TOPAS Drug'), :]
    basket_scores_long['variable'] = basket_scores_long['variable'].str.split('.').str.get(-1)
    basket_scores_long.to_csv(out_file, sep='\t')


def extract_basket_member_z_scores_4th_gen(results_folder: Union[str, Path],
                                  patient_regex: str,
                                  baskets_file: Union[str, Path]) -> None:
    """
    ...
    TODO: refactor initial dataframe loading into separate function shared with compute_basket_scores_4th_gen()
    """
    ...
    for basket_name, basket_df in all_baskets_df.groupby('BASKET'):
        ...
        for subbasket_name, subbasket_df in basket_df.groupby('subbasket_level'):
            scoring_rule = subbasket_df['SCORING RULE'].iloc[0].lower()
            if scoring_rule == 'highest z-score':
                subbasket_z_scores = get_weighted_z_scores(z_scores_fp_df, subbasket_df, 'Gene names', 'GENE NAME')
            elif scoring_rule == 'highest z-score (p-site)':
                subbasket_z_scores = get_weighted_z_scores(z_scores_pp_df, subbasket_df, 'Modified sequence', 'MODIFIED SEQUENCE')
            elif scoring_rule == 'highest protein phosphorylation score (2nd level z-score, fh)':
                subbasket_z_scores = get_weighted_z_scores(protein_phosphorylation_df, subbasket_df, 'Gene names', 'GENE NAME')
            elif scoring_rule == 'highest kinase score (2nd level z-score, fh)':
                subbasket_z_scores = get_weighted_z_scores(kinase_scores_df, subbasket_df, 'PSP Kinases', 'GENE NAME')
            else:
                raise ValueError(f"Unknown scoring rule {scoring_rule}")

            subbasket_z_scores.index = subbasket_z_scores.index.map(lambda x: f'{x} - {subbasket_name}')

            z_score_dfs.append(subbasket_z_scores)

        z_score_df = pd.concat(z_score_dfs).T
        z_score_df = z_score_df.drop(z_score_df[z_score_df.index.str.startswith('targets')].index)
        z_score_df.index = z_score_df.index.str.replace(r'-R\d{1}', '', regex=True)
        z_score_df.to_csv(f'{results_folder}/basket_member_z_scores_{basket_name.replace("/", "_").replace(" ", "_")}.tsv', sep='\t')
...
if __name__ == '__main__':
    ...
    parser.add_argument("-m", "--basket-member-zscores", action='store_true',
