...
class LFQLoader(DataLoader):
    def __init__(self, evidence_files):
        self.evidence_files = ['/home/mgander/Atlantic/data/PP_evidence.txt']
    ...
    def load_data(self, use_cols: List[str]):
        ...
        """
        use_cols = [c for c in use_cols if not c.startswith('Reporter intensity corrected')]
        ...
        evidence_file = pd.read_csv(self.evidence_files[0], usecols=use_cols, delimiter='\t')

        if 'Gene names' not in columns and '|' in evidence_file['Proteins'][0]:
            evidence_file['Proteins'] = evidence_file['Proteins'].apply(keep_protein_name_from_fasta_header)
            evidence_file['Leading proteins'] = evidence_file['Leading proteins'].apply(keep_protein_name_from_fasta_header)
        ...
        df = evidence_file.astype({'Reverse': 'category',
                                   'Experiment': 'category',
        ...
        cohort_name=''
        df['Batch'] = df['Experiment'].apply(lambda x: f'{x}')
