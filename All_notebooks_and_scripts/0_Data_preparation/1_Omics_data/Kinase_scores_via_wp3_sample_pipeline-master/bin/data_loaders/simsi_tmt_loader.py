...
from .. import simsi
from .data_loader import extract_batch_name, extract_experiment_name, test_batch_names_equals
...
class SimsiTMTLoader(TMTLoader):
    ...
    def load_data(self, use_cols: List[str]):
        simsi_evidence_file = simsi.find_simsi_evidence_file(self.results_folder, self.simsi_folder, self.data_type)
        all_batches = _parse_simsi_evidence_file(simsi_evidence_file, self.evidence_files, use_cols)
        return all_batches


def _parse_simsi_evidence_file(simsi_evidence_file: Union[str, Path, io.BytesIO],
                               evidence_files: List[Union[str, Path]],
                               use_cols: List[str]) -> List[pd.DataFrame]:
    ...
    experiment_to_batch_name_dict = {extract_experiment_name(f): extract_batch_name(f) for f in evidence_files}
