...
def _parse_evidence_file(evidence_file: Union[str, Path], use_cols: List[str]) -> pd.DataFrame:
    ...
    df = df.loc[~df.filter(regex='^Reporter intensity corrected').isnull().all(axis=1), :]
...
def _impute_ms1_intensity(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ref_channels = df[['Reporter intensity corrected 10', 'Reporter intensity corrected 11']]
    df['MS1 corrected reference intensity'] = ref_channels.mean(axis=1) / tmt_channels.sum(axis=1) * df['MS1']

    imputation_df = df.groupby('Modified sequence')['MS1 corrected reference intensity'].apply(lambda x: x.mean()).reset_index(
        name='Mean MS1 corrected reference intensity')
    ...
    df.loc[missing_ms1 & has_reference, 'MS1'] \
        = df['Mean MS1 corrected reference intensity'] * tmt_channels.sum(axis=1) / ref_channels.mean(axis=1)

    # TODO: impute MS1 for scans with no MS1 and no reference channel values by using summed channel intensities (does not seem to happen, at least not in PP)

    num_ms1_before = (~missing_ms1).sum()
    num_imputed = (missing_ms1 & has_reference & ~np.isnan(df['Mean MS1 corrected reference intensity'])).sum()
    logger.info(f"Found {num_ms1_before} scans with MS1.")
    logger.info(f"Imputed MS1 values for {num_imputed} additional scans (+{round(num_imputed / num_ms1_before * 100, 1)}%)")
